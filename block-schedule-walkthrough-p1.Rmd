...
# Block Schedule Walkthrough, Part 1
...
After a bit of research, I decided to use a [genetic algorithm](https://en.wikipedia.org/wiki/Genetic_algorithm) for this project. However, while I had a conceptual understanding of how they worked, I had never used - much less built - a genetic algorithm. This document - along with part two - was written to document and walk through my code for this project.
...
### Step 1: Initial Schedule Creation
...
Then, we run a loop over each of the blocks to the calculate fitness scores for each schedule. For each student, we determine whether one of their intended sections is being offered in that block. And, if not, whether any intended courses - but different sections - are being offered in that block. If a student could register for an intended section, they receive a score of 0. If the student could not, their score is based on the number of intended courses - but different sections - for which they could register. For example, if a student couldn't register for any of their intended sections but there is another section of intended course being offered, they would receive a score of 0.5. And, if a student couldn't register for any of their intended sections but there are other sections for two other intended courses being offered, they would receive a score 0.33. 
...
This concludes the first portion of the scheduling program. In Part 2, we will walk through the second half of the scheduling program wherein we will introduce variation through crossover and mutation events as part of future generations. 
